...
# İndirdiğin JSON dosyasının tam ve kusursuz hali
firebase_sifre = {
...
def login():
    ...
    rol = request.form.get('role') # Formdan öğrenci mi öğretmen mi olduğu gelir

    try:
        # Firebase deposundaki 'kullanicilar' koleksiyonunda bu kişiyi ara
        kullanicilar_ref = db.collection('kullanicilar')
        sorgu = kullanicilar_ref.where('kullanici_adi', '==', kullanici).where('sifre', '==', sifre).where('rol', '==', rol).stream()
        
        giris_basarili = False
        for doc in sorgu:
            giris_basarili = True
            break # Eşleşen biri varsa döngüden çık
...
@app.route('/ogretmen')
def ogretmen_paneli():
    # Sana önceki mesajda attığım eksiksiz öğretmen paneli (ogretmen.html)
